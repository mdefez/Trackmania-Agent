...
class TrackmaniaTrack:
    ...
    def _draw_blocks(self):
        ...
        for idx, row in self.df.iterrows():
            ...
            if "Start" in row['Block'] :
                plt.fill(corners[:,0], corners[:,1], color='lightgreen', edgecolor='black', label = "Start")
            elif "Finish" in row['Block'] :
                plt.fill(corners[:,0], corners[:,1], color='lightcoral', edgecolor='black', label = "Stop")
            else :
    ...
    def plot_distance_to_next_curve_field(self, density=6):
        self._draw_blocks()
        ...
        for idx, row in self.df.iterrows():
            ...
            for i in range(density):
                for j in range(density):
                    sample_x = cx - self.block_size/2 + (i + 0.5) / density * self.block_size
                    sample_z = cz - self.block_size/2 + (j + 0.5) / density * self.block_size

                    d = self.distance_to_next_curve([sample_x, sample_z])
                    if d is not None:
                        xs.append(-sample_x)   # match your drawing convention
                        zs.append(sample_z)
                        values.append(d)

        sc = plt.scatter(xs, zs, c=values, cmap='viridis', s=20)
        plt.colorbar(sc, label="Distance to next curve")
        plt.title("Distance to Next Curve Field")
        plt.xlabel("X")
        plt.ylabel("Z")
        plt.axis('equal')
        plt.grid(True)
    ...
    def angle_to_line(self, direction_vector, pos) :
        car_angle = self._compute_car_cangle(direction_vector)
        angle = self.compute_line_angle(pos)
    ...
    def compute_line_angle(self, pos) :
        ...
        # print("Computed line angle (deg):", angle_deg)
        ...
        for idx, row in self.df.iterrows():
            ...
            for i in range(density + 1):
                for j in range(density + 1):
                    ...
                    angle_rad = self.compute_line_angle([sample_x, sample_z])
                    # print(angle_deg)
                    if angle_rad is not None:
                        # angle_rad = np.radians(angle_deg)
                        dx = 2 * np.cos(angle_rad)
                        dz = 2 * np.sin(angle_rad)  
                        plt.arrow(-sample_x, sample_z, dx, dz, head_width=1.0, head_length=1.5, fc='blue', ec='blue', alpha=0.6)
        plt.title("Direction vector field")
        plt.show()
    ...
    def distance_to_next_curve(self, pos):
        ...
        # print("returning -1")
        return 100  # No more curves ahead
    ...
    def _compute_car_cangle(self, direction_vector):
        _fixed_direction = [direction_vector[0], direction_vector[2]]
        ## Compute oriented car angle
        # print("fixed direction :", _fixed_direction)
        car_angle = np.arccos(np.dot(np.array([-1, 0]), _fixed_direction) / (np.linalg.norm(np.array([-1,0])) * np.linalg.norm(_fixed_direction)))
        ## Determine sign
        cross_prod = np.cross(np.array([-1,0]), _fixed_direction)
    ...
    def distance_to_median_line(self, pos):
        ...
        if 'Curve' in current_block['Block']:
            ...
            dist_to_median = radius - dist_to_center
            if current_block['Rotate'] == 'Left':
                dist_to_median = -dist_to_median
        else:
            ## Straight block
            block_center = np.array([-current_block['cx'], current_block['cz']])
            to_center_vec = corrected_pos - block_center
            if current_block['angle'] == 180 :
                dist_to_median = to_center_vec[1]  # Assuming median line
            elif current_block['angle'] % 180 == 0:
                # East-West
                dist_to_median = -to_center_vec[1]  # Assuming median line
            else:
                # North-South
                dist_to_median = to_center_vec[0]  # Assuming median line
        
        return dist_to_median
    
    ## Test if reched finish line
    def is_finished(self, pos):
        idx = self._conpute_block_idx(pos)
        if idx is None:
            return False
        return idx == len(self.df) - 1
...
if __name__ == "__main__":
    ...
    print("Angle to line:", np.degrees(track.compute_line_angle([528, 720])))
    print("Angle to line:", np.degrees(track.compute_line_angle([528,810])))
    print("Angle of line:", np.degrees(track.compute_line_angle([450,916])))

    # North
    print("car direction North:", np.degrees(track._compute_car_cangle(np.array([0,0,1]))))
    # East
    print("car direction East:", np.degrees(track._compute_car_cangle(np.array([-1,0,0]))))
    # South
    print("car direction South:", np.degrees(track._compute_car_cangle(np.array([0,0,-1]))))
    # West
    print("car direction West:", np.degrees(track._compute_car_cangle(np.array([1,0,0]))))

    ## Angles to line
    print("Angle to line:", np.degrees(track.angle_to_line(np.array([0,0,1]), [528,820])))
    print("Angle to line:", np.degrees(track.angle_to_line(np.array([0,0,-1]), [528,700])))
    ...
    # track.plot_sampled_angles(10)

    print("Line angle:", np.degrees(track.compute_line_angle([530, 805])))

    # track.plot_distance_to_next_curve_field()