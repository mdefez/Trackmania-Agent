...
class RefLineEnvContinuous(TMEnv) :
    ...
    def reward(self, data) -> float :
        ...
        # print("line_proximity:", line_proximity)
        line_procimity_term = line_proximity * 20 * 0
        angle_to_line_term = - (abs(self.angle_to_median_line) * 10) ** 2
        dist_to_line_term = abs(self.distance_to_median_line) * 10 * -1

        # print("Distance to line:", d)
        # print("Angle to line:", median_line_angle)

        # print(f"self.angle_to_median_line: {self.angle_to_median_line}")
        # print(f"distance to median line: {self.distance_to_median_line}")

        _cooked_feature_term = - (self.distance_to_median_line * self.angle_to_median_line * 2) ** 2

        r = self.speed * (np.cos(median_line_angle) - abs(distance_to_line) / 10)
        print("Line angle", median_line_angle)
        print(f"line_proximity: {line_procimity_term}, dist_term: {dist_to_line_term:.3f}, angle_term: {angle_to_line_term:.3f}, d_dist term: {d_dist_term:.3f}, speed_term: {speed_term:.3f}, cooked_feature: {_cooked_feature_term:.3f}")
        # return d_dist_term + _cooked_feature_term + speed_term + dist_to_line_term + angle_to_line_term
        return r * 10 + d_dist_term * 10