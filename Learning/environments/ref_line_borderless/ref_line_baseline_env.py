...
class RefFLineBaseLineEnv(TMEnv) :
    def __init__(self, track_path_csv: str) :
        super().__init__(observation_space=gym.spaces.Box(low=-np.inf, high=np.inf, shape=(5,), dtype=np.float32))
        self.history = []
    ...
    def _is_terminated(self, data):
        ...
        if self.track.is_finished([self.x, self.y]) :
            self.need_save = True
            print("Terminated: finished line")
            return True
    ...
    def _data_to_observation(self, data):
        ...
        position = [vdata["position"][0], vdata["position"][2], vdata["position"][1]]
    ...
    def reward(self, data) -> float :
        ...
        if self.z < Z_LIMIT :
            print("Fell off")
            return - abs(np.sin(self.angle_to_median_line)) * 100
        ...
        if self.track.is_finished([self.x, self.y]) :
            print("Time out")
            self.need_save = True
            return 10 * self.elapsed_time / 1000
        
        if len(self.speed_history) >= HISTORY_LENGTH and max(self.speed_history) < SPEED_HISTORY_THRESHOLD :
            print("Stuck")
            return -250 + (400 - self.track.compute_distance_to_finish(position)) * 2
        ...
        paper_term = self.speed * (2 * np.cos(median_line_angle) - abs(distance_to_line) / 10) * 10
